export const paymentGatewayAccordionItems = [
  {
    index: 0,
    title: 'Plugin Ready',
    image: 'payment_gateway_tab_1',
    imageAlt: '',
    to: { title: 'Learn more about Plugins', link: '/platforms' },
    content:
      '<div class="max-md:text-mobile-body-normal">\n' +
      '        Connect to top eCommerce platforms and access advanced payment and\n' +
      '        AI-powered fraud protection through QashPay.\n' +
      '      </div>\n' +
      '      <ul class="list-[square] pl-15">\n' +
      '        <li>\n' +
      '          <span class="max-md:text-mobile-body-normal">\n' +
      '            Simplify your tech stack\n' +
      '          </span>\n' +
      '        </li>\n' +
      '        <li>\n' +
      '          <span class="max-md:text-mobile-body-normal">\n' +
      '            Boost sales conversions\n' +
      '          </span>\n' +
      '        </li>\n' +
      '        <li>\n' +
      '          <span class="max-md:text-mobile-body-normal">\n' +
      '            Improve the checkout experience\n' +
      '          </span>\n' +
      '        </li>\n' +
      '      </ul>',
  },
  {
    index: 1,
    title: 'Fraud Prevention',
    image: 'payment_gateway_tab_2',
    imageAlt: '',
    to: {
      title: 'Learn more about Fraud Prevention',
      link: '/fraud-management',
    },
    content:
      '<div class="max-md:text-mobile-body-normal">\n' +
      '        Protect your customers and reduce false positives with fast, automated\n' +
      '        deployment of advanced machine learning and fraud detection models.\n' +
      '      </div>\n' +
      '      <ul class="list-[square] pl-15">\n' +
      '        <li>\n' +
      '          <span class="max-md:text-mobile-body-normal">\n' +
      '            Boost fraud detection rates by up to 22%\n' +
      '          </span>\n' +
      '        </li>\n' +
      '        <li>\n' +
      '          <span class="max-md:text-mobile-body-normal">\n' +
      '            Extend the accuracy and effectiveness of existing models\n' +
      '          </span>\n' +
      '        </li>\n' +
      '        <li>\n' +
      '          <span class="max-md:text-mobile-body-normal">\n' +
      '            Respond to fraud and behavioral changes in real time\n' +
      '          </span>\n' +
      '        </li>\n' +
      '      </ul>\n' +
      '      <div class="max-md:text-mobile-body-normal">\n' +
      '        QashPay helps you stay ahead of evolving threats with intelligent,\n' +
      '        adaptive protection.\n' +
      '      </div>',
  },
  {
    index: 2,
    title: 'Pay by Link',
    image: 'payment_gateway_tab_3',
    imageAlt: '',
    to: { title: 'Learn more about Pay by Link', link: '/payment-links' },
    content:
      '<div class="max-md:text-mobile-body-normal">\n' +
      '        Send one-click payment links or QR codes that lead customers directly to\n' +
      '        your hosted payment form.\n' +
      '      </div>\n' +
      '\n' +
      '      <ul class="list-[square] pl-15">\n' +
      '        <li>\n' +
      '          <span class="max-md:text-mobile-body-normal">\n' +
      '            Generate and send payment requests instantly\n' +
      '          </span>\n' +
      '        </li>\n' +
      '        <li>\n' +
      '          <span class="max-md:text-mobile-body-normal">\n' +
      '            Maximize all payment channels\n' +
      '          </span>\n' +
      '        </li>\n' +
      '        <li>\n' +
      '          <span class="max-md:text-mobile-body-normal">\n' +
      '            Reach more customers with ease\n' +
      '          </span>\n' +
      '        </li>\n' +
      '      </ul>',
  },
]
