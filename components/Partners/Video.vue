<template>
  ...
  >
    ...
    <AppButton
      class="group absolute bottom-12 left-12 flex items-center !gap-6 bg-white hover:bg-[#C1B7DA]/40 focus:bg-white focus:!text-primary-400 !text-primary-300 shadow-[0px_4px_4px_rgba(0,0,0,0.05)]"
      @click="toggleVideo"
    >
    ...
    <AppButton
      class="absolute bottom-12 right-12 bg-primary-300 opacity-40 hover:opacity-80 hover:!bg-primary-300 focus:opacity-100 focus:!bg-primary-300 !text-primary-300 !rounded-full !w-20 !h-20 !max-w-20 !max-h-20 !p-0"
      @click="muted = !muted"
    >
      <template #icon:end>
        <Icon v-if="muted" mode="svg" name="app-icon:mute-icon" />
        <Icon v-else mode="svg" name="app-icon:unmute-icon" />
      </template>
    </AppButton>
  </div>
</template>
